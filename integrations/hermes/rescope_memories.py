...
def cmd_archive_namespace(conn: sqlite3.Connection, namespace: str, apply: bool) -> None:
    ...
    by the live bots. Idempotent: only touches active rows."""
    n = conn.execute(
    ...
    if apply:
        conn.execute(
            "UPDATE memories SET state='archived' WHERE namespace=? AND state='active'", (namespace,)
        )
        conn.commit()
...
def main() -> None:
    ...
    try:
        ...
        if not (args.to_shared or args.always_pin or args.delete_staging or args.archive_namespace):
            print("nothing to do — use --list, --to-shared, --always-pin, "
                  "--delete-staging, or --archive-namespace")
    finally:
        conn.close()
    if not args.apply and (
        args.to_shared or args.always_pin or args.delete_staging or args.archive_namespace
    ):
